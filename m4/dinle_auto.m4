dnl Some convenience macros for the build system, ripped off
dnl from MPD sources.

AC_DEFUN([DINLE_AUTO_ENABLED], [
	var="enable_$1"
	feature="$2"
...
dnl very similar to DINLE_AUTO_RESULT, but does not finalize the
dnl detection; it assumes that more checks will follow.
AC_DEFUN([DINLE_AUTO_PRE], [
	name="$1"
	var="enable_$1"
	found="found_$name"
	feature="$2"
	...
	if eval "test x`echo '$'$var` != xno" && eval "test x`echo '$'$found` = xno"; then
                DINLE_AUTO_DISABLED([$name], [$feature], [$msg])
	fi
])

AC_DEFUN([DINLE_AUTO_RESULT], [
	name="$1"
	var="enable_$1"
	found="found_$name"
	feature="$2"
	...
	if eval "test x`echo '$'$found` = xyes"; then
                DINLE_AUTO_ENABLED([$name], [$feature])
	else
                DINLE_AUTO_DISABLED([$name], [$feature], [$msg])
	fi
])

AC_DEFUN([DINLE_AUTO_PKG], [
